...
"updateParamOfSource",function(b,a,c){return function(f,e,k,g){var l=[],l=f?f.split(" "):["mixer","oscillator","oneshot","audio","stream"];angular.forEach(l,function(l){var f=l.match(/oscillator(0|1)/);null!==f?b(parseInt(f[1]),e,k,g):e?a(l,e,k,g):c(l,k,g)})}}]);h.value("openDialog",function(b,a,c,f,e){$("<div />").html(e).dialog({title:b,autoOpen:!0,show:"explode",hide:"explode",width:a,height:c,modal:f,draggable:!0,resizable:!1,zIndex:9999,buttons:{OK:function(){$(this).dialog("close").dialog("destroy").remove()}}})});
...
a).slice(-2)},a=new Date,c=a.getFullYear(),f=b(a.getMonth()+1),e=b(a.getDate()),k=b(a.getHours()),g=b(a.getMinutes()),b=b(a.getSeconds());return c+f+e+k+g+b});h.value("getCurrentPatches",function(){return{mastervolume:X("oscillator").params().mastervolume,glide:{type:X("oscillator").params().oscillator.glide.type,time:X("oscillator").params().oscillator.glide.time},oscillator0:X("oscillator").params().oscillator.oscillator0,oscillator1:C("oscillator").params().oscillator.oscillator0,eg:{attack:X("oscillator").params().eg.attack,
...
k=f.max;var g=f.step,l=f.range||"min",n=f.animate||"slow",h=f.source,m=f.module,p=f.param,t=m?m+"-"+p:p,q=function(a){b(h,m,p,a);$(".spinner-"+t).each(function(b,d){var c=$(d).parents("ui-spinner");c.attr("source")===h&&c.attr("module")===m&&("glide"!==m&&"time"===p&&(a*=Math.pow(10,3)),$(d).spinner("value",a))})};$(c).children("div").addClass("slider-"+t).slider({value:parseFloat(a),min:parseFloat(e),max:parseFloat(k),step:parseFloat(g),range:l,animate:n,change:function(a,b){var d=parseFloat(b.value);
"glide"!==m&&"time"===p&&(d/=Math.pow(10,3));q(d)},slide:function(a,b){var d=parseFloat(b.value);"glide"!==m&&"time"===p&&(d/=Math.pow(10,3));q(d)}})}}}]);h.directive("uiSpinner",["updateParam",function(b){return{restrict:"EA",template:"<input />",link:function(a,c,f,e,k){a=f.value;e=f.min;k=f.max;var g=f.step,l=f.source,n=f.module,h=f.param,m=n?n+"-"+h:h,p=function(a){b(l,n,h,a);$(".slider-"+m).each(function(b,u){var c=$(u).parent("ui-slider");c.attr("source")===l&&c.attr("module")===n&&("glide"!==
n&&"time"===h&&(a*=Math.pow(10,3)),$(u).slider("value",a))})};$(c).children("input").addClass("spinner-"+m).spinner({min:parseFloat(e),max:parseFloat(k),step:parseFloat(g),spin:function(a,b){var c=parseFloat(b.value);"glide"!==n&&"time"===h&&(c/=Math.pow(10,3));p(c)}}).spinner("value",parseFloat(a))}}}]);h.directive("uiSliderTime",["$rootScope","$timeout","createTimeString",function(b,a,c){return{restrict:"EA",template:"<div></div>",link:function(f,e,k,g,l){g=k.value;l=k.max;var n=k.step,h=k.range||
"min";k=k.animate||"slow";$(e).children("div").attr("id","slider-audio-current-time").slider({value:parseFloat(g),min:0,max:parseFloat(l),step:parseFloat(n),range:h,animate:k,change:function(a,b){},slide:function(c,l){X("audio").param("currentTime",parseFloat(l.value));X("audio").module("analyser").domain("time-all-L").update(X("audio").param("currentTime"));X("audio").module("analyser").domain("time-all-R").update(X("audio").param("currentTime"));a(function(){b.currentTime=X("audio").param("currentTime")})}});
f.$watch(function(){return f.duration},function(a){$("#slider-audio-current-time").slider("option","max",a);$("#spinner-audio-current-time").spinner("option","max",a)});f.$watch(function(){return b.currentTime},function(a){$("#slider-audio-current-time").slider("value",a);$("#spinner-audio-current-time").spinner("value",a);f.currentTimeText=c(a)})}}}]);h.directive("uiSpinnerTime",["$rootScope","$timeout",function(b,a){return{restrict:"EA",template:"<input />",link:function(c,f,e,k,
g){c=e.value;k=e.max;e=e.step;$(f).children("input").attr("id","spinner-audio-current-time").spinner({min:0,max:parseFloat(k),step:parseFloat(e),spin:function(c,e){X("audio").param("currentTime",parseFloat(e.value));X("audio").module("analyser").domain("time-all-L").update(X("audio").param("currentTime"));X("audio").module("analyser").domain("time-all-R").update(X("audio").param("currentTime"));a(function(){b.currentTime=X("audio").param("currentTime")})}}).spinner("value",parseFloat(c))}}}]);h.directive("uiToggleEffector",
["sources",function(b){return{restrict:"A",link:function(a,c,f,e,k){var g=f.effector,l=parseInt(f.state);0!==l&&1!==l||angular.forEach(b,function(a){X(a).module(g).state(Boolean(l));C("oscillator").module(g).state(Boolean(l));X(a).module(g).state()?$(c).addClass("check-on"):$(c).removeClass("check-on")});$(c).on("click",function(){angular.forEach(b,function(a){X(a).module(g).state()?(X(a).module(g).state(!1),C("oscillator").module(g).state(!1),$(c).removeClass("check-on")):(X(a).module(g).state(!0),
C("oscillator").module(g).state(!0),$(c).addClass("check-on"))})})}}}]);h.directive("uiModalProgress",[function(){return{restrict:"EA",link:function(b,a,c,f,e){$(a).css({position:"fixed",top:"0px",left:"0px","z-index":999,padding:"15% 0%",width:"100%",height:"100%","background-color":"rgba(0, 0, 0, 0.6)"})}}}]);h.directive("uiTooltip",function(){return{restrict:"A",link:function(b,a,c,f,e){$(a).tooltip({tooltipClass:"ui-tooltip-custom"})}}});h.directive("uiSortable",function(){return{restrict:"A",
link:function(b,a,c,f,e){$(a).sortable({axis:"x",cancel:"fieldset, :input, button, label, a, #piano",cursor:"move",opacity:.6})}}});h.directive("uiSelect",["$timeout","sources",function(b,a){return{restrict:"A",link:function(c,f,e,k,g){$(f).select2();switch($(f).attr("id")){case "select-sound-source":$(f).on("change",function(){var a=this.value;switch(a){case "oscillator":X("stream").stop();b(function(){c.currentSoundSource=a;c.selectedSoundSource=a});break;case "one-shot":c.isEnableOneshot&&b(function(){c.currentSoundSource=
a;c.selectedSoundSource=a});X("stream").stop();break;case "stream":try{X("stream").start(),X("stream").module("session").start(),b(function(){c.selectedSoundSource=a})}catch(e){openDialog("Error",400,"auto",!0,"<p><b>"+e.message+"</b></p>")}break;case "midi":X("stream").stop()}});break;case "select-rec-track":$(f).on("change",function(){if(!c.isActive){var l=parseInt(this.value);b(function(){c.activeTrack=l});angular.forEach(a,function(a){"oscillator"!==a&&"audio"!==a&&(X(a).module("recorder").ready(l),
...
c,f,e,k){var g=f.model;$(c).hide();a.$watch(function(){return b.isActives},function(a){a[g]?$(c).slideDown(1E3,"easeOutQuint"):$(c).slideUp(1E3,"easeOutQuint")},!0)}}}]);h.directive("audioUploader",["$timeout","readFile","readFileByDragAndDrop","readFileProgressCallback","readFileErrorCallback",function(b,a,c,f,e){return{restrict:"A",link:function(k,g,l,n,h){var m={event:null,type:"ArrayBuffer",success:function(a,c){X("audio").ready.call(X("audio"),c);b(function(){k.isModalProgressReadFile=!1})},
error:function(a,b){e(b)},progress:function(a){f(k,b,a)}};g[0].addEventListener("change",function(b){m.event=b;b=a(m);this.value="";b instanceof Blob&&$(g).next('[type="text"]').val(b.name)},!1);l=$(g).next('[type="text"]')[0];c(l,m)}}}]);h.directive("rirUploader",["$timeout","sources","readFile","readFileByDragAndDrop","readFileProgressCallback","readFileErrorCallback",function(b,a,c,f,e,k){return{restrict:"A",link:function(g,l,h,r,m){var p={event:null,type:"ArrayBuffer",success:function(c,e){angular.forEach(a,
function(a){X(a).module("reverb").start.call(X(a).module("reverb"),e)});b(function(){g.isModalProgressReadFile=!1})},error:function(a,b){k(b)},progress:function(a){e(g,b,a)}};l[0].addEventListener("change",function(a){p.event=a;a=c(p);this.value="";a instanceof Blob&&$(l).next('[type="text"]').val(a.name)},!1);h=$(l).next('[type="text"]')[0];f(h,p)}}}]);h.directive("mmlUploader",["$timeout","readFile","readFileByDragAndDrop","readFileProgressCallback","readFileErrorCallback",function(b,a,c,f,e){return{restrict:"A",
link:function(k,g,l,h,r){var m={event:null,type:"Text",success:function(a,c){var e=function(){b(function(){k.mml=c;k.paused=!0;k.readyMML()})};""===k.mml?e():$("<div />").html("<p><b>MML already exists. Overwrite ?</b></p>").dialog({title:"Confirmation",autoOpen:!0,show:"explode",hide:"explode",modal:!0,width:500,height:"auto",draggable:!0,resizable:!1,zIndex:9999,buttons:{OK:function(){e();$(this).dialog("close").dialog("destroy").remove()},CANCEL:function(){$(this).dialog("close").dialog("destroy").remove()}}});
b(function(){k.isModalProgressReadFile=!1})},error:function(a,b){e(b)},progress:function(a){f(k,b,a)}};g[0].addEventListener("change",function(b){m.event=b;b=a(m);this.value="";b instanceof Blob&&$(g).next('[type="text"]').val(b.name)},!1);l=$(g).next('[type="text"]')[0];c(l,m)}}}]);h.controller("XSoundController",["$rootScope","$scope","$window","$timeout","BASE_URL","sources","openDialog","getCurrentPatches",function(b,a,c,f,e,k,g,l){l=[e+"one-shot/piano-2/C.wav",e+"one-shot/piano-2/D.wav",e+"one-shot/piano-2/E.wav",
e+"one-shot/piano-2/F.wav",e+"one-shot/piano-2/G.wav",e+"one-shot/piano-2/A.wav",e+"one-shot/piano-2/B.wav"];var h=function(a){switch(parseInt((a+9)%12)){case 0:case 1:return 0;case 2:case 3:return 1;case 4:return 2;case 5:case 6:return 3;case 7:case 8:return 4;case 9:case 10:return 5;case 11:return 6}},r=function(){for(var a=Array(88),b=0;88>b;b++){var c={buffer:0,rate:1,loop:!1,start:0,end:0,volume:1};c.buffer=h(b);var d=c,e=b,l=-1!==[1,4,6,9,11,13,16,18,21,23,25,28,30,33,35,37,40,42,45,47,49,52,
54,57,59,61,64,66,69,71,73,76,78,81,83,85].indexOf(e)?!0:!1,f=0;0<=e&&2>=e?f=.0625:3<=e&&14>=e?f=.125:15<=e&&26>=e?f=.25:27<=e&&38>=e?f=.5:39<=e&&50>=e?f=1:51<=e&&62>=e?f=2:63<=e&&74>=e?f=4:75<=e&&86>=e?f=8:87<=e&&98>=e&&(f=16);l&&(f*=Math.pow(2,1/12));d.rate=f;a[b]=c}return a},m=function(){f(function(){a.isReady=!0})},p=function(){var b=[];angular.forEach(a.rirs,function(c,e){X.ajax(c.url,6E4,function(d,c){X.decode(X.get(),c,function(d){b.push(d);a.$apply(function(){a.progress=Math.floor(b.length/
a.rirs.length*100);a.progressStyle.width=a.progress+"%"});b.length===a.rirs.length&&(angular.forEach(k,function(a){X(a).module("reverb").preset(b)}),C("oscillator").module("reverb").preset(b),m())},function(){m()})},function(a,b){g("Error",400,"auto",!0,"<p><b>The loading of RIRs failed.</b></p>")})})};try{X("oneshot").setup({resources:l,settings:r(),timeout:6E4,success:function(b,c){a.isEnableOneshot=!0;0===a.rirs.length?m():p()},error:function(a,b){g("Error",400,"height",!0,"<p><b>The loading of audio files failed.</b></p>")},
...
b.isActive=!1;b.tracks=[1,2,3,4];b.activeTrack=0;b.objectURL="";b.finename="";angular.forEach(c,function(a){"oscillator"!==a&&"audio"!==a&&X(a).module("recorder").setup(b.tracks.length)});b.controlRecord=function(a){a.preventDefault();b.isActive?g():k()};b.createWAVE=function(a){a.preventDefault();b.isActive||(a=b.$parent.selectedSoundSource,"oscillator"===a&&(a="mixer"),b.objectURL=X(a).module("recorder").create("all",16,2,"blob"),b.objectURL&&(a=new Audio(b.objectURL),a.setAttribute("controls",
...
function(b,c){b.preventDefault();a.channel=c};a.selectAPI("canvas")}]);h.controller("MMLController",["$rootScope","$scope","$http","$timeout","BASE_URL","createDateTimeString",function(b,a,c,f,e,k){var g=[0,1,2,3];b={start:function(b,c){"oscillator"===a.$parent.currentSoundSource&&X("mixer").mix([X("oscillator"),C("oscillator")]);if("R"!==b.indexes[c]){var e=b.indexes[c];f(function(){a.$parent.isSoundStops[e]=!1})}},stop:function(b,c){if("R"!==b.indexes[c]){var e=b.indexes[c];f(function(){a.isSoundStops[e]=
!0})}},ended:function(){angular.forEach(g,function(a){X("oscillator",a).state(!1);C("oscillator",a).state(!1)});a.readyMML();a.paused=!0;a.$parent.clearKeyboards()},error:function(a,b){}};X("mml").setup(b);C("mml").setup(b);a.paused=!0;a.mml="";a.dataURL="";a.filename="";a.error="";a.$watch(function(){return a.$parent.currentSoundSource},function(b){a.readyMML()});c.get(Math.floor(2*Math.random())?e+"mml/mml-foreverlove.txt":e+"mml/mml-tears.txt").success(function(b,c,e,f){a.mml=b;a.typeMML()}).error(function(a,
b,c,e){});a.typeMML=function(b){a.readyMML();a.paused=!0};a.readyMML=function(){var b=a.mml.split(/\|+/);"oscillator"===a.$parent.currentSoundSource?(0<b.length&&(X("mml").ready(X("oscillator"),b[0]),C("mml").ready(C("oscillator"),[])),1<b.length&&C("mml").ready(C("oscillator"),b[1])):X("mml").ready(X("oneshot"),b)};a.controlMML=function(b){b.preventDefault();X("mml").isSequences()&&(b=X("mml").get(),a.paused?("oscillator"===a.$parent.currentSoundSource?(angular.forEach(g,function(a){X("oscillator",
a).state(!0);C("oscillator",a).state(!0)}),a.$parent.oscillators.oscillator0.isActive=!0,a.$parent.oscillators.oscillator1.isActive=!0,0<X("mml").get().length&&X("mml").start(0),0<C("mml").get().length&&C("mml").start(0),X("mixer").module("recorder").start(),X("mixer").module("session").start()):(angular.forEach(b,function(a,b){X("mml").start(b)}),X("oneshot").module("recorder").start(),X("oneshot").module("session").start()),a.paused=!1):(X("mml").stop(),C("mml").stop(),angular.forEach(g,function(a){X("oscillator",
a).state(!0);C("oscillator",a).state(!0)}),a.paused=!0))};a.stopMML=function(b){X("mml").stop();C("mml").stop();a.readyMML();a.paused=!0;a.$parent.clearKeyboards()};a.downloadMML=function(b){0===a.mml.trim().length?b.preventDefault():(a.dataURL=X("mml").download(a.mml),a.filename="mml-"+k()+".txt")}}]);h.controller("SessionController",["$scope","$location","$timeout","sources","openDialog",function(b,a,c,f,e){var k="",g="";-1!==a.host().indexOf("localhost")?(k="localhost",g=""):-1!==a.host().indexOf("korilakkuma.github.io")&&
(k="210.152.156.200",g="/home/node/websocket/");var h=function(a,f){e("Confirmation",800,"auto",!1,"<p><b>Connection to ("+f.url+") succeeded.</b></p>");c(function(){b.isActive=!0})},n=function(a,f){e("Confirmation",400,"auto",!0,"<p><b>Connection closed.</b></p>");c(function(){b.isActive=!1})},r=function(a,f){e("Error",400,"auto",!0,"<p><b>Server Error.</b></p>");c(function(){b.isActive=!1})},m=function(){c(function(){b.$parent.isModalProgressSessionWait=!1;X("mixer").module("session").state()?b.isActive=
!0:b.isActive=!1})},p=function(a){b.$parent.isModalProgressSessionWait=!0};b.isActive=!1;b.url="";b.toggleSessionState=function(a){b.isActive=!b.isActive;if(b.isActive)if(X("mixer").module("session").isConnected()&&X("oneshot").module("session").isConnected()&&X("audio").module("session").isConnected()&&X("stream").module("session").isConnected())angular.forEach(f,function(a){"oscillator"!==a&&X(a).module("session").state(!0,m,p)});else{var c={tls:!1,host:k,port:8E3,path:g,open:h,close:n,error:r};
try{angular.forEach(f,function(a){"oscillator"!==a&&(X(a).module("session").setup(c).state(!0,m,p),c.port++)})}catch(u){b.isActive=!1,e("Error",400,"auto",!0,"<p><b>"+u.message+"</b></p>")}}else angular.forEach(f,function(a){"oscillator"!==a&&X(a).module("session").state(!1,m,p)})}}]);h.controller("PatchController",["$rootScope","$scope","$http","$location","$timeout","sources","openDialog","getCurrentPatches",function(b,a,c,f,e,h,g,l){var n;n=-1!==f.host().indexOf("localhost")?"http://localhost:3000":
"http://curtaincall.c.node-ninja.com:3000";var r=function(){a.isDisabled=!0;c({method:"POST",url:n+"/account/delete",timeout:1E4,withCredentials:!0,params:"",data:{username:a.deleteAccounts.username,password:a.deleteAccounts.password,_csrf:a.csrf},headers:{"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8"},transformRequest:function(a){return $.param(a)}}).success(function(b,c,d,e){angular.isObject(b)&&(a.csrf=b.csrf,a.isAuth=b.isAuth,a.authedUsername=b.username,a.isAuth?angular.forEach(b.message,
function(a){g("Error",400,"auto",!1,"<p><b>"+a+"</b></p>")}):angular.forEach(b.message,function(a){g("Confirmation",400,"auto",!1,"<p><b>"+a+"</b></p>")}));a.isDisabled=!1}).error(function(b,c,d,e){q();a.isDisabled=!1})},m=function(b){if(angular.isObject(b)){var c=function(a,b){angular.forEach(h,function(c){X(c).module(a).state(b)});C("oscillator").module(a).state(b);b?$('[effector="'+a+'"]').addClass("check-on"):$('[effector="'+a+'"]').removeClass("check-on")};"mastervolume"in b&&$(".slider-masterVolume:not(:first)").slider("value",
b.mastervolume);if("glide"in b){var d=b.glide;"time"in d&&$(".slider-glide-time").slider("value",d.time)}"eg"in b&&(d=b.eg,"attack"in d&&$(".slider-eg-attack").slider("value",d.attack),"decay"in d&&$(".slider-eg-decay").slider("value",d.decay),"sustain"in d&&$(".slider-eg-sustain").slider("value",d.sustain),"release"in d&&$(".slider-eg-release").slider("value",d.release));if("oscillator0"in b){var f=b.oscillator0;"state"in f&&e(function(){a.$parent.oscillators.oscillator0.isActive=!f;a.$parent.toggleOscillatorState(null,
0)});"type"in f&&e(function(){a.$parent.changeWaveType(null,0,f.type)});"gain"in f&&$(".slider-volume:first").slider("value",f.gain);"octave"in f&&$(".slider-octave:first").slider("value",f.octave);"fine"in f&&$(".slider-fine:first").slider("value",f.fine)}if("oscillator1"in b){var g=b.oscillator1;"state"in g&&e(function(){a.$parent.oscillators.oscillator1.isActive=!g;a.$parent.toggleOscillatorState(null,1)});"type"in g&&e(function(){a.$parent.changeWaveType(null,1,g.type)});"gain"in g&&$(".slider-volume:last").slider("value",
g.gain);"octave"in g&&$(".slider-octave:last").slider("value",g.octave);"fine"in g&&$(".slider-fine:last").slider("value",g.fine)}"compressor"in b&&(d=b.compressor,"state"in d&&c("compressor",d.state),"threshold"in d&&$(".slider-threshold").slider("value",d.threshold),"knee"in d&&$(".slider-knee").slider("value",d.knee),"ratio"in d&&$(".slider-ratio").slider("value",d.ratio),"attack"in d&&$(".slider-compressor-attack").slider("value",d.attack),"release"in d&&$(".slider-compressor-release").slider("value",
d.release));"distortion"in b&&(d=b.distortion,"state"in d&&c("distortion",d.state),"curve"in d&&$("#select-distortion-type").val(d.curve).trigger("change"),"samples"in d&&$("#select-distortion-sample").val("value",d.samples),"drive"in d&&$(".slider-distortion-drive").slider("value",d.drive),"color"in d&&$(".slider-distortion-color").slider("value",d.color),"tone"in d&&$(".slider-distortion-tone").slider("value",d.tone));"wah"in b&&(d=b.wah,"state"in d&&c("wah",d.state),"cutoff"in d&&$(".slider-wah-cutoff").slider("value",
d.cutoff),"depth"in d&&$(".slider-wah-depth").slider("value",d.depth),"rate"in d&&$(".slider-wah-rate").slider("value",d.rate),"resonance"in d&&$(".slider-wah-resonance").slider("value",d.resonance));"equalizer"in b&&(d=b.equalizer,"state"in d&&c("equalizer",d.state),"bass"in d&&$(".slider-equalizer-bass").slider("value",d.bass),"middle"in d&&$(".slider-equalizer-middle").slider("value",d.middle),"treble"in d&&$(".slider-equalizer-treble").slider("value",d.treble),"presence"in d&&$(".slider-equalizer-presence").slider("value",
d.presence));"filter"in b&&(d=b.filter,"state"in d&&c("filter",d.state),"type"in d&&$("#select-filter-type").val(d.type).trigger("change"),"frequency"in d&&$(".slider-filter-cutoff").slider("value",d.frequency),"Q"in d&&$(".slider-filter-Q").slider("value",d.Q),"gain"in d&&$(".slider-filter-gain").slider("value",d.gain),"attack"in d&&$(".slider-filter-attack").slider("value",d.attack),"decay"in d&&$(".slider-filter-decay").slider("value",d.decay),"sustain"in d&&$(".slider-filter-sustain").slider("value",
d.sustain),"release"in d&&$(".slider-filter-release").slider("value",d.release));"autopanner"in b&&(d=b.autopanner,"state"in d&&c("autopanner",d.state),"depth"in d&&$(".slider-autopanner-depth").slider("value",d.depth),"rate"in d&&$(".slider-autopanner-rate").slider("value",d.rate));"tremolo"in b&&(d=b.tremolo,"state"in d&&c("tremolo",d.state),"depth"in d&&$(".slider-tremolo-depth").slider("value",d.depth),"rate"in d&&$(".slider-tremolo-rate").slider("value",d.rate));"ringmodulator"in b&&(d=b.ringmodulator,
"state"in d&&c("ringmodulator",d.state),"depth"in d&&$(".slider-ringmodulator-depth").slider("value",d.depth),"rate"in d&&$(".slider-ringmodulator-rate").slider("value",d.rate));"phaser"in b&&(d=b.phaser,"state"in d&&c("phaser",d.state),"stage"in d&&$(".select-phaser-stage").val(d.stage),"frequency"in d&&$(".slider-phaser-cutoff").slider("value",d.frequency),"depth"in d&&$(".slider-phaser-depth").slider("value",d.depth),"rate"in d&&$(".slider-phaser-rate").slider("value",d.rate),"mix"in d&&$(".slider-phaser-mix").slider("value",
d.mix),"feedback"in d&&$(".slider-phaser-feedback").slider("value",d.feedback));"flanger"in b&&(d=b.flanger,"state"in d&&c("flanger",d.state),"time"in d&&$(".slider-flanger-time").slider("value",d.time),"depth"in d&&$(".slider-flanger-depth").slider("value",d.depth),"rate"in d&&$(".slider-flanger-rate").slider("value",d.rate),"mix"in d&&$(".slider-flanger-mix").slider("value",d.mix),"tone"in d&&$(".slider-flanger-tone").slider("value",d.tone),"feedback"in d&&$(".slider-flanger-feedback").slider("value",
d.feedback));"chorus"in b&&(d=b.chorus,"state"in d&&c("chorus",d.state),"time"in d&&$(".slider-chorus-time").slider("value",d.time),"depth"in d&&$(".slider-chorus-depth").slider("value",d.depth),"rate"in d&&$(".slider-chorus-rate").slider("value",d.rate),"mix"in d&&$(".slider-chorus-mix").slider("value",d.mix),"tone"in d&&$(".slider-chorus-tone").slider("value",d.tone),"feedback"in d&&$(".slider-chorus-feedback").slider("value",d.feedback));"delay"in b&&(d=b.delay,"state"in d&&c("delay",d.state),
"time"in d&&$(".slider-delay-time").slider("value",d.time),"dry"in d&&$(".slider-delay-dry").slider("value",d.dry),"wet"in d&&$(".slider-delay-wet").slider("value",d.wet),"tone"in d&&$(".slider-delay-tone").slider("value",d.tone),"feedback"in d&&$(".slider-delay-feedback").slider("value",d.feedback));"reverb"in b&&(b=b.reverb,"state"in b&&c("reverb",b.state),"dry"in b&&$(".slider-reverb-dry").slider("value",b.dry),"wet"in b&&$(".slider-reverb-wet").slider("value",b.wet),"tone"in b&&$(".slider-reverb-tone").slider("value",
b.tone));e(function(){a.currentPatches=l()})}},p=function(b){c({method:"POST",url:n+"/patch/update",timeout:1E4,withCredentials:!0,params:"",data:{name:b,patch:angular.toJson(l()),_csrf:a.csrf},headers:{"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8"},transformRequest:function(a){return $.param(a)}}).success(function(b,c,f,h){e(function(){angular.isObject(b)&&(a.csrf=b.csrf,a.isAuth=b.isAuth,a.patchLists=angular.fromJson(b.patches),angular.forEach(b.message,function(a){g("Confirmation",
500,"auto",!1,"<p><b>"+a+"</b></p>")}))})}).error(function(a,b,c,e){q()})},t=function(b){c({method:"POST",url:n+"/patch/delete",timeout:1E4,withCredentials:!0,params:"",data:{name:b,_csrf:a.csrf},headers:{"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8"},transformRequest:function(a){return $.param(a)}}).success(function(b,c,e,f){angular.isObject(b)&&(a.csrf=b.csrf,a.isAuth=b.isAuth,a.patchLists=angular.fromJson(b.patches),angular.forEach(b.message,function(a){g("Confirmation",500,
"auto",!1,"<p><b>"+a+"</b></p>")}))}).error(function(a,b,c,e){q()})},q=function(){g("Error",500,"auto",!1,"<p><b>Connection to Server failed.</b></p>")};a.isAuth=!1;a.isSignin=!0;a.isSignup=!1;a.isPatchList=!1;a.isSavePath=!1;a.isAccount=!1;a.isDisabled=!1;a.csrf="";a.signups={username:"",password:""};a.signins={username:"",password:""};a.deleteAccounts={username:"",password:""};a.authedUsername="";a.patchname="";a.patchLists=[];a.dialogPatchDetailId="dialog-patch-detail";a.isShowDialogPatchDetail=
!1;a.$watch(function(){return a.isAuth},function(b){b?(a.signups={username:"",password:""},a.signins={username:"",password:""},a.currentPatches=l(),a.toggleAction(null,"patchList")):(a.deleteAccounts={username:"",password:""},a.currentPatches={},a.patches={},a.patchname="",a.patchLists=[],a.authedUsername="",a.isShowDialogPatchDetail=!1,a.toggleAction(null,"signin"))});a.$on("onOpenPatchDialog",function(b){a.currentPatches=l()});a.cancelClose=function(a){a.stopImmediatePropagation()};a.toggleAction=
function(b,c){null!==b&&b.preventDefault();a.isSignin=!1;a.isSignup=!1;a.isPatchList=!1;a.isSavePatch=!1;a.isAccount=!1;switch(c){case "signin":a.isSignin=!0;break;case "signup":a.isSignup=!0;break;case "patchList":a.isPatchList=!0;break;case "savePatch":a.isSavePatch=!0;break;case "account":a.isAccount=!0}};a.signin=function(b){a.isDisabled=!0;c({method:"POST",url:n+"/account/signin",timeout:1E4,withCredentials:!0,params:"",data:{username:a.signins.username,password:a.signins.password,_csrf:a.csrf},
headers:{"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8"},transformRequest:function(a){return $.param(a)}}).success(function(b,c,e,f){angular.isObject(b)&&(a.csrf=b.csrf,a.isAuth=b.isAuth,a.authedUsername=b.username,a.isAuth?a.patchLists=angular.fromJson(b.patches):angular.forEach(b.message,function(a){g("Error",500,"auto",!1,"<p><b>"+a+"</b></p>")}));a.isDisabled=!1}).error(function(b,c,e,f){q();a.isDisabled=!1})};a.signup=function(b){a.isDisabled=!0;c({method:"POST",url:n+"/account/signup",
timeout:1E4,withCredentials:!0,params:"",data:{username:a.signups.username,password:a.signups.password,_csrf:a.csrf},headers:{"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8"},transformRequest:function(a){return $.param(a)}}).success(function(b,c,e,f){angular.isObject(b)&&(a.csrf=b.csrf,a.isAuth=b.isAuth,a.authedUsername=b.username,a.isAuth?angular.forEach(b.message,function(a){g("Confirmation",600,"auto",!1,"<p><b>"+a+"</b></p>")}):angular.forEach(b.message,function(a){g("Error",
500,"auto",!1,"<p><b>"+a+"</b></p>")}));a.isDisabled=!1}).error(function(b,c,e,f){q();a.isDisabled=!1})};a.signout=function(b){a.isDisabled=!0;c({method:"POST",url:n+"/account/signout",timeout:1E4,withCredentials:!0,params:"",data:{_csrf:a.csrf},headers:{"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8"},transformRequest:function(a){return $.param(a)}}).success(function(b,c,e,f){angular.isObject(b)&&(a.csrf=b.csrf,a.isAuth=b.isAuth,a.authedUsername=b.username);a.isDisabled=!1}).error(function(b,
c,e,f){q();a.isDisabled=!1})};a.deleteAccount=function(a){$("<div />").html('<p><b>Delete your account. OK ?<br /> <b class="dangerous">If you delete account, the all of patches that you saved are also deleted.</b></b></p>').dialog({title:"Confirmation",autoOpen:!0,show:"bounce",hide:"explode",modal:!1,width:650,height:"auto",draggable:!0,resizable:!1,zIndex:9999,buttons:{OK:function(){r();$(this).dialog("close").dialog("destroy").remove()},CANCEL:function(){$(this).dialog("close").dialog("destroy").remove()}}})};
a.typePatchname=function(b){a.patchname=b.currentTarget.value};a.savePatch=function(b){a.isDisabled=!0;c({method:"POST",url:n+"/patch/add",timeout:1E4,withCredentials:!0,params:"",data:{name:a.patchname,patch:angular.toJson(l()),_csrf:a.csrf},headers:{"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8"},transformRequest:function(a){return $.param(a)}}).success(function(b,c,e,f){angular.isObject(b)&&(a.csrf=b.csrf,a.isAuth=b.isAuth,angular.forEach(b.message,function(a){g("Confirmation",
500,"auto",!1,"<p><b>"+a+"</b></p>")}),b.result&&(a.patchLists=angular.fromJson(b.patches)));a.isDisabled=!1}).error(function(b,c,e,f){q();a.isDisabled=!1})};a.openPatchDetail=function(b,c){if(angular.isObject(c))a.patches=c.patch;else{var d=c.patch.replace(/&quot;/g,'"').replace(/&apos;/g,"'").replace(/&amp;/g,"&");a.patches=angular.fromJson(d)}a.isShowDialogPatchDetail=!0;$("#"+a.dialogPatchDetailId).dialog({title:c.name,autoOpen:!0,show:"explode",hide:"explode",modal:!1,width:600,height:400,draggable:!0,
resizable:!1,zIndex:9999,position:{my:"center top",at:"center top"},buttons:{LOAD:function(){m(a.patches);$(this).dialog("close")},UPDATE:function(){$("<div />").html("<p><b>Update the patch. OK ?</b></p>").dialog({title:"Confirmation",autoOpen:!0,show:"explode",hide:"explode",modal:!1,width:400,height:"auto",draggable:!0,resizable:!1,zIndex:1E4,buttons:{OK:function(){p(c.name);$(this).dialog("close").dialog("destroy").remove();$("#"+a.dialogPatchDetailId).dialog("close")},CANCEL:function(){$(this).dialog("close").dialog("destroy").remove()}}})},
DELETE:function(){$("<div />").html("<p><b>Delete the patch. OK ?</b></p>").dialog({title:"Confirmation",autoOpen:!0,show:"explode",hide:"explode",modal:!1,width:400,height:"auto",draggable:!0,resizable:!1,zIndex:1E4,buttons:{OK:function(){t(c.name);$(this).dialog("close").dialog("destroy").remove();$("#"+a.dialogPatchDetailId).dialog("close")},CANCEL:function(){$(this).dialog("close").dialog("destroy").remove()}}})},CLOSE:function(){$(this).dialog("close")}},close:function(b,c){$(this).dialog("destroy");
e(function(){a.isShowDialogPatchDetail=!1})}})};c({method:"GET",url:n,timeout:1E4,withCredentials:!0,params:"",headers:{}}).success(function(b,c,d,e){angular.isObject(b)&&b.result&&(a.isAuth=b.isAuth,a.csrf=b.csrf,a.authedUsername=b.username,a.isAuth&&(a.patchLists=angular.fromJson(b.patches)))}).error(function(a,b,c,e){q()})}])})();